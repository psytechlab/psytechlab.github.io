...
Транскрибатор для аудиозаписей сессий между психологом и клиентом. Обработка выполняется на локальном устройстве, данные остаются под вашим контролем. Работает под Windows. Поддерживает большое количество форматов: можете использовать как wav запись с физического диктофона, так и записи из zoom-подобных платформ. Записи можно ставить в очередь.

Разработка коммерческая по заказу проекта «Открытые двери».